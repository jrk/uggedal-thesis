...
This chapter includes information on our methods for analysing modern web
sites and for performing a user experiment on a prototypical social navigation
implementation (see \chapterref{implementation} for details).
...
\label{section:methodology.user.experiment}

As desccribed in \chapterref{implementation} we implemented a prototype of a
new navigational design for a well established web site.

We used a controlled experiment for testing how this navigational design
fared. A controlled experiemnt simply means that we're comparing results given
by an experimental group against a control group. The only difference between
these two groups are that the former are given the aspect we wanted to test
and the latter are not provided with this aspect. With paralells to medical
trails our control group are given a placebo.
For more detail about what the experiment and control group were given see
\sectionref{implementation.design.activity.stream} and
