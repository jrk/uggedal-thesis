...
\citep[\p{905}, \vol{15}]{simpson89} it's defined as:
...
\label{section:background.sociality.the.social.web.ggg}
...
to the Graph (\abbr{GGG}: Giant Global Graph) in a
...
\sidenote[-5]{
  ...
  many of Facebook's (see \sectionref{analysis.facebook} for more on Facebook)
  third party applications, Google's similar \project{OpenSocial} initiative
...
\sidenote{
  ...
  \postquote[\p{NNN}, \vol{NN}]{simpson89}{%
...
Active and direct social navigaton inherently make advice available by
...
\sidenote[-1]{
  ...
  Flickr\dash{}a social image sharing web site available at we dig deeper
  when we analyze its social navigation capabilities in
  \sectionref{analysis.flickr}.
}

\subsubsection{Tag sharing \oldand scope}
\label{section:background.social.navigation.applied.forms.tagging.sharing}
...
%%
%% Need read-trough from this point to the next marker.
%%

Colaborative filtering is used on several modern web sites. One example is
...
that one does not infer similarities amongst users (as in GroupLens).%

\begin{figure}
  \includegraphics[width=0.95\textwidth]{scrsh_reddit}
  \caption[Collaborative Filtering at Reddit]{
    ...
    the author (the up arrow is highlighted red) which gave the subsmission
...
As Reddit recently became open source we were able to take a peek into its
colaborative filtering functionality. We did this to dissect how collaborative
filtering algorithms are used for real applications. The source code of this
algorithm can be found in \sourcecoderef{reddit.cf.algorithm}. What follows is
the algorithm in mathematical notation.

Given the time the entry was posted $A$ and
the time of 7:46:43 a.m. December 8, 2005 $B$,
we have $t_s$ as their difference in seconds
\begin{align*}
  t_s &= A - B \\
  \intertext{and $x$ as the difference between the number of up votes $U$
             and the number of down votes $D$}
  x &= U - D \\
  \intertext{where $y \in \{-1, 0, 1\}$}
  y &= 
  \begin{cases} 
     1 & \text{if } x > 0 \\
     0 & \text{if } x = 0 \\
    -1 & \text{if } x < 0
  \end{cases} \\
  \intertext{and $z$ as the maximal value,
             of the absolute value of $x$ and $1$}
  z &= 
  \begin{cases} 
    |x| & \text{if } |x| \geq 1 \\
    1   & \text{if } |x| < 1
  \end{cases} \\
  \intertext{we have the rating as a function $f(t_s, y, z)$}
  f(t_s, y, z) &= \log_{10}{z} + \frac{yt_s}{45000}
\end{align*}

The resulting rating is a real number which is used for numericaly sorting
entries. This is neither a computationaly expensive algorithm nor a very
...
%%
%% Summarize a bit here from the previous material.
%%

\citet{svensson05} performed a throughout evaluation of the \project{Kalas}%
\sidenote{
  A system for sharing cooking receipes.
}
