...
Schachter, the creator of del.icio.us, highlight tagging as it's most
...
information it containts. If these objects are composed of video content tags
can serve as an augmentor for performing keyword based searches as one could
...
principle navigation mechanism. By doing so \citet[\p{2}]{foller06}
saw the need for bootstrapping the availability of tags so that users would be
more inclined to create their own tags. Their solution was to algoritmically
create tags based on the contents of Wikipedia and hoped the existance of
these tags would stimulate users to start tagging themselves.

Tagging have it's shorcomings. Tags could be misspelled, tags with the same
name are not always  homonymous, and tags with the same meaning does not always
have the same name because of synonyms \citep[\p{59}]{aurnhammer06}. In
...
\sidenote{
  ...
  \emph{movies}. An algoritm that generates the semantic meaning of
...
\citep[\p{946}]{li07}.
The problem of browsing large scale tagging collections is tackled by infering
a hierarchy%
\sidenote{
  ...
  hierarchy from the variying levels of abstraction in such tags saying
