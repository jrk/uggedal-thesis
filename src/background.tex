...
\label{section:background.navigation.on.the.web}

The Web is based on the ideas of \emph{hypertext}--a term coined by
...
One of the two available forms of social navigation found in ``Knowledge
Sea''--a digital educational library --is recommendations by item annotation
\citep[p.~13]{brusilovsky05}. Users can leave their emphatic marks on content
and thereby specify it's usefulness. Questionnaires showed that a fair
majority of users was agreeable to the use of such recommendations
...
\sectionref{section:background.social.navigation.applied.forms.recommendations}.
The system also incorporates what the authors calls \emph{traffic-based}
\citep[p.~12]{brusilovsky05}
social navigation--in other words history based visualizations in the form of
read-wear. The access of different articles in the digital library are
...
Our idea of implementing additional features on a web site came from the
author's involment in an underground community based arround the Ruby%
\sidenote{
  Available at \url{http://ruby-lang.org}
}
programming language. Hoodwink.d, as it's called, is underground in the sence
that it's quite hard to get in to ``the club''. It's members rarely talk about
Hoodwink.d publicly, baring similarities to the underground fight club in the
motion picture aptly named \emph{Fight Club}%
\sidenote{
  ...
  second rule of Fight Club is--you \emph{do not} talk about Fight Club.''
}.
