...
\sidenote{
  ...
  provides. The underlying principle of Google's search algorithm is
  that a page is rated of importance based on how many pages who
...
\postquote{powazek08}{therefore sees collective intelligence as}{%
...
be compareble in accuracy as similar articles in
...
and in what setting information is written. Wikipedia consists in many cases
of remixed information taken from other sources where references soemtimes is
lacking. When you read a published article, a book or even a personal web page
you get insight into who the author is. In wikipedia you're only provided a
username of the person who's added a particular item of textual information.
...
\project{del.icio.us}
\sidenote{
  Initially a system for organizing Joshua Scachter's personal bookmark
...
\sidefigure{Blogroll}{%
  ...
  political blogs where people provide news and oppinion from a
...
\term{tags} and the act of applying them is called \term{tagging}. Joshua
Schachter, the creator of del.icio.us, hightlight tagging as it's most
essential feature\dash{}the feature that set it apart from the competition
\cite[\p{225}]{livingston07}.
...
\emph{folk} and \emph{taxonomy}. A folksonomy is a stricktly bottom-up
approach because of the lack of any predefined taxanomic structure. They
therefore rely on \postquote[\p{31}]{marlow06}{%
  shared and emergent social structures and behaviours, as well as related and
  linguistic structures of the user community}
Since we're mainly interested in the navigational possibilites tagging can
give us we're leaving out a deeper discussion of the benefits and drawbacks
of folksonomies. \citet{golder06} and \citet{marlow06} gives a datailed
...
\sidenote{%
  ...
  we analyse it's social navigation capabilities in
...
than to come up with their own. In other vords our vocabulary is influenced
by the user community. \citet[\p{186}]{sen06} confirmed our personal
observations when they found that the community incluence affects the
...
Under such cirumstances \citet[\p{185}]{sen06} found that the probability
of using a previously used tag rose as the ammount of tags the user had
...
you gave it and the tags you chose to apply. Since a bookmark is distinguishe
...
tags for this single item. There is then only one repesentation of this item
and it's tags.
Tags need not be collaboratively created. When one for instance are tagging
one's personal email messages it makes sence to keep such behaviour private.

As we've seen folksonomies can be seperated by their level of tag sharing
(private systems, fully open systems, and systems with user controll over
what gets shared) and tag scope (are taggs applied to an item globally
or do they belong to seperate users).
...
aggregator with tagging support. They designed their application with a foucus
on enabeling users to easily find relevant syndication items trough
social navigation. The cornerstone of their approach to social navigation was
enabeling a folksonomy the users could navigate for finding syndication
items. Sadly user studies and evaluations
of the merrits of this approach to social navigation is non-existent.

\citet{millen06} on also gives an account of how they used collaborarive
tagging with a focus on enabeling social navigation in their
\project{Dogear} social bookmarking system. Fortunately they have conducted
studies on the merrits of such an approach to social navigation. When users
...
the usefullness of folksonomies for enabeling social navigation. In addition
...
Advanced algoritms known in the field of collaborative filtering
and recommendation systems have been used together with folksonomies
consisting of collaborative tags
\citep[\pp{112}{113}]{wu06}. Prelimnary studies have shown positive
results when harnessing such social knowledge with filtering algoritms
as opposed to traditional folksonomy representation
\citep[\p{114}]{wu06}. Such a folksonomy could hovewer interfere with an
existing recommendation system. \citet[\p{190}]{sen06} found that the
introduciton of tagging and tag display in their established
...
folksonomy was more likely to be present for users faimiliar with the old
