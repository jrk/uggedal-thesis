...
or form controls%
\sidenote{
  Form controls such an input field, with a button for activating it, is often
  used for search.
}.
...
More specifically, we either using a strategy of \term{browsing} or
\term{searching} when we're navigating the Web.
...
Serendipity%
\sidenote{
  \prequote[\p{631}]{andel94}{defines serendipity as}{%
...
people \citep[\p{101}]{lin07}. The Web is becoming democratized
...
\sidenote{
  SixDegrees closed their operations in 2000. It's founder believes it was
  simply ahead of its time \citep{boyd07}.
}
...
all its features, \figureref{scrsh.sixdegrees.welcome} shows an early
...
\sidenote{
  For instance Blink (\url{http://blink.dagbladet.no}) was a popular
...
\sidenote{
  Facebook is now the largest social network in our home country, Norway.
  As of May 14, 2008 it had approximately 1,142,300 registered profiles
  where the user had associated his or her self with the Norway network.
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{scrsh_sixdegrees_welcome}
...
Mash-ups is often created by utilizing one or more web \abbr{API}s%
\sidenote{
  \abbr{API} is short for Application Programming Interface,
  \prequote[\p{2}]{jacobsen91}{%
    A set of calling conventions defining how a service is
    invoked through a software package}
}.
\citet[\p{86}]{floyd07} argues that web APIs enable innovation since they
...
\sidenote[-7\onelineskip]{
  ...
  mash-ups between open web services.
},
...
As \citet[\p{806}]{dieberger97} argues the Web's growth, even at its modest
...
\sidenote{%
  ...
  we analyze its social navigation capabilities in
...
\sidenote{
  ...
  visualization seem to have come from \citet{flanagan03} in his display
...
\figureref{scrsh.tagcrowd.journal.redflavor}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{scrsh_tagcrowd_journal_redflavor}
...
\citet[\p{996}]{rivadeneira07} defines the tasks a tag cloud can support:
  \begin{inparaenum}[(i)]
...
\citet[\p{18}]{sinclair08} conducted a study into whether tag clouds provided
...
\citet[\pp{22}{23}]{sinclair08} found that the majority of users utilized the
...
\citeyearpar[\p{24}]{sinclair08}. Another drawback when finding more specific
information is that tag clouds are not as good at narrowing down information
\citeyearpar[\p{28}]{sinclair08}.
...
Tagging have its shortcomings. Tags could be misspelled, tags with the same
...
to be almost as accurate as explicit ratings, suggesting implict ratings can
be used successfully without introducing the additional overhead
explicit ratings embodies.
...
\citet[\pp{242}{243}]{herlocker00} gives compelling reasons for explaining
...
the way collaborative filtering \postquote[\p{241}]{herlocker00}{%
...
\term{item-based} recommendation systems repsectively
\citep[\p{156}]{greco04}.

Recommender systems can be based on either one or both of these two approaches
...
But recommender systems have not only been troughoutly used in research
...
\sidenote{
  An online store primarily carrying books, available at
  url{http://amazon.com}.
}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{scrsh_amazon_recommendation}
...
\sidenote{
  An American online movie rental company recided at
...
seemed to complement each other \citep[\p{4}]{bell07}.
...
% results
%
% these seem to be more concerned with the search results

As we've seen scientific work seem to mainly be concerned with introducing
social navigation in search results, not the search itself.
It can be argued that using a search engine like Google is a form of social
navigation since rankings of search results are based on the behavior of other
people as they create links between different web pages. Automated systems can
also create links, so wheter Google search provides social navigationthis is
a grey zone. Anyhow, Google have an interesting product
called \project{Google Suggest}. As can be seen in
\figureref{scrsh.google.suggest} search terms are suggested as you type.
It's not quite clear how the suggestive search terms are created, but it seems
like they are retrieved from other user's past searches. If this holds true
Google Suggest can be seen as a truly social search tool.

\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{scrsh_google_suggest}
  \caption[Google Suggest]{
    Google Suggest\dash{}suggestive search interface,
    retrieved May 21, 2008, from
    \url{http://www.google.com/webhp?complete=1&hl=en}.
  }
  \label{figure:scrsh.google.suggest}
\end{figure}
