...
as it's called, is underground in the sence that it's quite hard to get in to
...
\sidenote{
  The obfuscation can be removed either by turning off CSS support
  in your browser or viewing the page's source.
}
one have to be pretty technical sawy to deciper what the information 
...
\sectionref{implementation.architecture.client.side.plattform})
that enables the functionality
of Hoodwink.d.%
\sidenote{
  Oh no, we just broke the first and second rule of \work{Fight Club}!
}
This approach creates a barrier to enty which only allows the most
