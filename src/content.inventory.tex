...
The result of this exercise were a table identifying a web site's various
pages and the relationships amongst them.
...
When collecting data one notices that there is lots of pages
...
listings and so on. I therefore introduced several such variables.
...
The inventory table is structured with an identifier representing the
hierarchical nature of the captured page. The page title is taken note of in
addition to the name of the link that was used to navigate to this page and
this link's location. Due to a slight ambiguity of title usage on Flickr
we decided to collect the most representative sample%
\sidenote{A choice between the \texttt{<title>} element in the \texttt{<head>}
          of the HTML document and the \texttt{<h1>} top level heading in-line
          the \texttt{<body>} portion of the document was made.}.
If this resulted in unsatisfactory results we created a new title using the
best of our abilities to make it as clear and descriptive as possible.
