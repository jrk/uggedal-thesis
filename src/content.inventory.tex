...
\sidenote{
  A system that adheres to the principles of REST
  (Representational State Transfer)
  \citep[p.~76]{fielding00} are sometimes called
  RESTful. Such principles can be applied to URLs
  making them represet \emph{resources}
  \citep[p.~110]{fielding00}.
}
The sites' structure was
clearly illustrated by the hierarchy of directories represented in the URL. We
therefore took a not of these URLs when we collected our sample data, but
choose not to include them here because we're not convinced of their
usefullnes in light of navigation.
...
\sidenote{
  ...
  associated with fake accounts (e.g. Paris Hilton).'' \citep{facebook08}
}
and the account have to represent an existing individual%
\sidenote{
  ``using the name of a group or organization is not permitted, as
  Facebook accounts are for individual use only.'' \citep{facebook08}
}.
