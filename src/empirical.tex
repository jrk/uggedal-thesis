...
conducted a follow-up survey of our respondents to gauge wether they managed
...
\begin{items}
  ...
  \item Firefox user{}?\dash{}selection between the following frequence of use
  ...
  \item How often do you use \urort{}?\dash{}selection between the following
    ...
    and seldom/never.
  \item Do you sign-in (with username and password) when using
...
\begin{items}
  \item How frequently have you used \latest{} when you are
    ...
    frequence of use categories: have not used, only a few times, almost
    every time, and every time.
\end{items}

Next we wanted to investigate the percieced usefullness and ease of use for
...
We shorted the statements of percieved usefulness down to four alternatives
which we felt made sence for our implementation:
...
\begin{items}
  ...
  \item \latest{} would be usefull for keeping up-to-date on favorites.
\end{items}

The statement for percieved ease of use was also shorted down to four
alternatives:

\begin{items}
  \item It would be easy to learn to use \latest{}.
  \item It would be easy to make \latest{} do what I want.
  \item It would be easy to become skillfull at using \latest{}.
...
\begin{items}
  \item How many favorites do you have on \urort{}?\dash{}a numerical value
    of the ammount of favorites the respondent had was expected.
  \item What makes you add artist on \urort{} as favorites?\dash{}the
    ...
    own. The preset resons were: the artist's \emph{music},
...
how easy it was for them to keep up-to-date on favorites. These statements
was to be ranked on a 5-point Likert scale \citep{likert32}
(fully disagree, somewhat disagree, neither agree or disagree, somewhat agree,
fully disagree):
