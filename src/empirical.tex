...
\subsubsection{Wilcox signed-rank test}

The Wilcox signed-rank test is a non-parametric test for comparing two
related conditions and is an non-parametric alternative to the
dependent t-test \citep[\p{534}]{field05}.
The assumptions of the Wilcox signed-rank test are similar to those
...
\subsubsection{An activity stream will increase number of favorites}
...
\begin{table}[!ht]
  ...
  \end{tabular}
  \caption[Up-to-date on Favorites]{%
    How many favorites do you have on \urort{}?
  }
  \label{table:favorite.usage.number}
\end{table}


\subsection{Keeping up-to-date on activities}

In the following results we have graded respondents answers with integers from
1 to 5:
...
\begin{table}[!ht]
  ...
  \end{tabular}
  \caption[Up-to-date on Favorites]{%
    It's easy to keep up-to-date on what my favorites are doing
    on \urort{}.
  }
  \label{table:uptodate.favorites}
\end{table}
...
\begin{table}[!ht]
  ...
  \end{tabular}
  \caption[Up-to-date on Favorites' Songs]{%
    It's easy to keep up-to-date on whether my favorites publishes
    new songs on \urort{}.
  }
