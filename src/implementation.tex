...
\abbr{RECORD} project it became clear that \urort{} would be an excellent
...
\begin{figure}
  \includegraphics[width=\textwidth]{scrsh_hoodwinkd_obfuscation}
...
\sidenote[-4]{
  ...
  clearly describes the attitude members have towards outsiders:
  \begin{inparaenum}[(i)]
    \item you don't talk about fight club,
    \item you don't talk about fight club
  \end{inparaenum}
  \citep[\pp{48}{50}]{palahniuk96}.
}
...
\sectionref{selection.stack.client.platform} for details)
...
and thereby filtering out \term{newbies}.

\begin{figure}
  \begin{whole}
    \includegraphics[width=\wholewidth]{scrsh_hoodwinkd_comment_source}
...
\begin{figure}
  \begin{whole}
    \includegraphics[width=\wholewidth]{scrsh_socialthing}
...
through an activity stream seemed a good fit for \urort{}. But
...
technically easy%
\sidenote{
  Easy as in only requiring access to the \urort{} web pages.
}
to retrieve. Those that matched our criteria was:

\begin{items}
  \iterm{Songs} being published by the artist.
  \iterm{Song reviews} being written by users.
  \iterm{Concerts} being performed by the artist.
  \iterm{Blog posts} being written by the artist.
\end{items}
...
aged content. As activities were sorted chronologically in our stream we simply
...
\latest{}\dash{}emphasizing the focus on provided the latest activities
...
\begin{figure}
  \includegraphics[width=\textwidth]{scrsh_urort_activity_feed}
...
their favorites had activities present in the stream no such list was displayed.
\figureref{scrsh.urort.activity.feed} shows the listing of one such artist
without recent activity.

\begin{figure}
  \includegraphics[width=\textwidth]{scrsh_urort_favorite_list}
...
The reason for creating a robust application is twofold. Firstly we're
developing
