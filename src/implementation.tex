...
small script presented in \sectionref{source.code.javascript.comment.stripper}
...
\end{lstlisting}
  \caption[Parsing with Beautiful Soup]{%
          HTML parsing in Python with Beautiful Soup}
  \label{sourcecode:parsing.beautiful.soup}
\end{sourcecode}
...
\begin{lstlisting}
  html/'p > em.someclass'
\end{lstlisting}
  \caption[Parsing with Hpricot]{%
          HTML parsing in Ruby with Hpricot}
  \label{sourcecode:parsing.hpricot}
\end{sourcecode}
...
documents over \abbr{HTTP}. \executable{open-uri} is trvial to use and
integrated nicely with Hpricot:
...
# => "Eivind Uggedal"
\end{lstlisting}
  \caption[Fetching and Parsing with Hpricot and open-uri]{%
  ...
  \label{sourcecode:fetching.openuri.parsing.hpricot}
\end{sourcecode}
