...
% no new data. making existing data more redily available.
...
is handeled on the server side, while data display obviously is handeled on
the client side.

Based on the design decitions we've taken we needed to figure out what parts
...
encounters abnormal behaviour or bugs it's much easier to locate them when one
have sources available and one can trivially (depending on the complexity of
the prolem) create a patch that sortes them out.
...
that enables us to display new informaition trough navigational designs on the
\urort web site.
...
When doing protoype work it's important that the programming language one uses
is efficient to work with. This means that programmer efficiency is more
important than computational efficiency (a language's native performance).
Since we didden't have time to invest in learning a new language we had to do
with those we knew from before. Of those \project{Ruby}%
\sidenote{
  Ruby recides at \url{http://ruby-lang.org}.
},
\project{Python}%
\sidenote{
...
% write about benefits of concise source code
...
The core library we need is one that handels data extraction from existing
...
the availability of such a library and it's usefulles. Since we've already
revealed Ruby as our implementation language we're killing the suspence.
...
\sidenote{
  ...
  Hoodwink.d\dash{}our inspiration for a transparent protoype implementation.
}
and makes \abbr{HTML} parsing almost a fun endevor in our oppinion.
...
\sidenote{
  ...
  \term{\abbr{XHTLM}} on the other hand, a reformulated version of
...
\sidenote{
  Hyper Text Transfer Protocol. The data transfer p
}
framework is needed to make the
collected data available for our client.

% modest needs. serve json. maby include sub sub section before this one
